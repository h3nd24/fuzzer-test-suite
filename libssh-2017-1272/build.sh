...
build_lib() {
  rm -rf BUILD
  cp -rf SRC BUILD
  (cd BUILD && mkdir build && cd build && cmake -DCMAKE_SYSTEM_PROCESSOR="i386" -DCMAKE_C_COMPILER="$CC" -DCMAKE_CXX_COMPILER="$CXX" -DCMAKE_C_FLAGS="$CFLAGS" -DCMAKE_CXX_FLAGS="$CXXFLAGS" -DWITH_STATIC_LIB=ON  .. &&  make -j $JOBS)
}
