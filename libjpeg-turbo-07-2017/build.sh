...
build_lib() {
  rm -rf BUILD
  cp -rf SRC BUILD
#  (cd BUILD && autoreconf -fiv && ./configure --host=i686-pc-linux-gnu --without-simd LDFLAGS="-m32" && make -j $JOBS)
  (cd BUILD && autoreconf -fiv && ./configure --host=i686-pc-linux-gnu LDFLAGS="-m32" && make -j $JOBS)
}
